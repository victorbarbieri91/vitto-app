      ...
      >
        ...
        <div className="flex items-center px-3 py-2.5 gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-slate-800 truncate">{transaction.descricao}</p>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="text-[11px] text-slate-400">{transactionDate}</span>
              {transaction.total_parcelas != null && (
                <span className="text-[10px] text-blue-600 bg-blue-50 px-1 py-px rounded font-medium">
                  {transaction.parcela_atual}/{transaction.total_parcelas}
                </span>
              )}
              {transaction.is_fixed && (
                <span className="text-[10px] text-amber-600 bg-amber-50 px-1 py-px rounded font-medium">Fixa</span>
              )}
            </div>
          </div>
          {/* Value */}
          <span className="text-sm font-semibold text-slate-800 whitespace-nowrap tabular-nums flex-shrink-0">
